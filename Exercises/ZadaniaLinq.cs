...
public sealed class ZadaniaLinq
{
    ...
    {
        bool exists = DaneUczelni.Zapisy.Any(p => !p.CzyAktywny);
        return new List<string> { exists ? "True" : "False" };
    }
    ...
    public IEnumerable<string> Zadanie12_ParyStudentPrzedmiot()
    {
        return DaneUczelni.Zapisy
            ...
            .Join(
                ...
                t => t.zapis.PrzedmiotId,
                przedmiot => przedmiot.Id,
                (t,przedmiot) => $"{t.Name} - {przedmiot.Nazwa}"
                );
    }
    ...
    public IEnumerable<string> Zadanie14_SredniaOcenaNaPrzedmiot()
    {
        return DaneUczelni.Zapisy.Join(
            ...
            .Where(pair => pair.z.OcenaKoncowa.HasValue)
            .GroupBy(pair => pair.p.Nazwa)
            .Select(g => $"{g.Key} {g.Average(pair=> pair.z.OcenaKoncowa)}");
    }
    ...
    public IEnumerable<string> Zadanie15_ProwadzacyILiczbaPrzedmiotow()
    {
        return DaneUczelni.Prowadzacy.GroupJoin(
                ...
                pr => pr.Id,
                p => p.ProwadzacyId,
                (pr, pList) => new { pr.Imie, pr.Nazwisko, Count = pList.Count() }
            )
            .GroupBy(x => new { x.Imie, x.Nazwisko })
            .Select(g => $"{g.Key.Imie} {g.Key.Nazwisko}: {g.Sum(x => x.Count)}");
    }
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        return DaneUczelni.Studenci.Join(
            ...
            .Where(pair => pair.z.OcenaKoncowa.HasValue)
            .GroupBy(pair => (pair.s.Imie, pair.s.Nazwisko))
            .Select(g => $"{g.Key.Imie} {g.Key.Nazwisko} {g.Max(pair => pair.z.OcenaKoncowa)}");
    }
